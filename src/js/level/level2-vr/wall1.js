WHVR.tunnelWall1 = (function (rootNode) {
    var DTHETA = 20;
    var NUM_SEGMENTS = 5;

    var NEAR_CLIPPING_PLANE = 25;

    var GRADIENT_START = WHVR.LINE_OF_SIGHT;
    var GRADIENT_STOP  = GRADIENT_START - WHVR.BARRIER_SPACING;
    ...
    return {
        init: function (rootNode) {
            ...
            gradientStart.setAttribute('stop-color', '#000');
            ...
            gradientStop.setAttribute('stop-color', '#fff');
        ...
        updateDOM: function (x, y, offset) {
            ...
            for (var i = 0; i < NUM_SEGMENTS; i++) {
                ...
                var scale_x_ = 0.1 * WHVR.PROJECTION_PLANE_DISTANCE * (1 - x_ / WHVR.TUNNEL_RADIUS)
                             / (Math.tan(Math.PI * WHVR.FIELD_OF_VIEW / 360.0) * NEAR_CLIPPING_PLANE);
                var scale_y_ = 0.1 * WHVR.PROJECTION_PLANE_DISTANCE
                             / (Math.tan(Math.PI * WHVR.FIELD_OF_VIEW / 360.0) * NEAR_CLIPPING_PLANE);

                var skew = 180 * Math.atan(y_ / WHVR.TUNNEL_RADIUS) / Math.PI;
