WH.tunnelWall = (function (rootNode) {
    var DTHETA = 20;
    var NUM_SEGMENTS = 5;

    var NEAR_CLIPPING_PLANE = 25;

    var GRADIENT_START = WH.LINE_OF_SIGHT;
    var GRADIENT_STOP  = GRADIENT_START - WH.BARRIER_SPACING;
    ...
    return {
        ...
        updateDOM: function (x, y, offset) {
            ...
            for (var i = 0; i < NUM_SEGMENTS; i++) {
                ...
                var scale_y_ = 0.1 * WH.PROJECTION_PLANE_DISTANCE
                             / (Math.tan(Math.PI * WH.FIELD_OF_VIEW / 360.0) * NEAR_CLIPPING_PLANE);

                var skew = 180 * Math.atan(y_ / WH.TUNNEL_RADIUS) / Math.PI;
